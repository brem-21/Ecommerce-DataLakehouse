...
def test_load_data_from_s3_success(spark):
    ...
    with patch('pyspark.sql.readwriter.DataFrameReader.csv') as mock_read_csv:
        ...
        mock_read_csv.assert_called_once_with(f"s3a://{bucket_name}/{folder_path}", header=True, inferSchema=True)
...
def test_load_data_from_s3_failure(spark, caplog):
    ...
    with patch('pyspark.sql.readwriter.DataFrameReader.csv') as mock_read_csv:
        with caplog.at_level(logging.ERROR):
            ...
            assert "Failed to load data from s3a://test-bucket/raw-data/test/: S3 read error" in caplog.text
...
def test_transform_order_items_data(spark):
    ...
    expected_data = [
        ...
        (2, 10001, 1991, 15, 989, 2, "Reorder", "2025-04-02T12:30:00", "2025-04-02", "12:30:00")
    ]
    expected_df = spark.createDataFrame(expected_data, expected_schema)

    transformed_df = transform_order_items_data(input_df)

    assert transformed_df.schema == expected_schema, f"Schema mismatch: {transformed_df.schema}"
...
def test_join_datasets_with_products(spark):
    ...
    order_items_data = [
        (1, 1, 1990, 10, 988, 1, "Not_Reorder", "2025-04-01T11:27:00", "2025-04-01", "11:27:00"),
        (2, 2, 1991, 15, 989, 2, "Reorder", "2025-04-02T12:30:00", "2025-04-02", "12:30:00")
    ]
    ...
    expected_data = [
        (1, 1, 1990, 10, 988, "Apple", 1, "Produce", 1, "Not_Reorder", 123, 45.6, "2025-04-01T11:27:00", "2025-04-01", "11:27:00"),
        (2, 2, 1991, 15, 989, "Bread", 2, "Bakery", 2, "Reorder", 124, 67.8, "2025-04-02T12:30:00", "2025-04-02", "12:30:00")
    ]
    expected_df = spark.createDataFrame(expected_data, expected_schema)
...
def test_join_datasets_without_products(spark):
    ...
    expected_data = [
        (1, 1, 1990, 10, 988, 1, "Not_Reorder", "2025-04-01T11:27:00", "2025-04-01", "11:27:00", 123, 45.6),
        (2, 2, 1991, 15, 989, 2, "Reorder", "2025-04-02T12:30:00", "2025-04-02", "12:30:00", 124, 67.8)
    ]
    expected_df = spark.createDataFrame(expected_data, expected_schema)
...
def test_normalize_data(spark):
    ...
    expected_departments_df = spark.createDataFrame(expected_departments_data, expected_departments_schema)
    ...
    expected_order_items_df = spark.createDataFrame(expected_order_items_data, expected_order_items_schema)

    users_df, departments_df, products_df, orders_df, order_items_df = normalize_data(input_df)

    assert users_df.collect() == expected_users_df.collect(), "Users data mismatch"
    assert departments_df.collect() == expected_departments_df.collect(), "Departments data mismatch"
    assert products_df.collect() == expected_products_df.collect(), "Products data mismatch"
    assert orders_df.collect() == expected_orders_df.collect(), "Orders data mismatch"
    assert order_items_df.collect() == expected_order_items_df.collect(), "Order items data mismatch"
...
def test_partition_data_with_sort(spark):
    ...
    result_df = partition_data(input_df, partition_by="date", num_partitions=2, sort_by="order_id")
...
def test_save_normalized_data_to_s3_with_partition(spark, caplog):
    ...
    with patch('pyspark.sql.dataframe.DataFrame.write', new_callable=PropertyMock, return_value=mock_writer):
...
def test_save_master_data_to_s3(spark):
    ...
    with patch('pyspark.sql.dataframe.DataFrame.write', new_callable=PropertyMock) as mock_write:
