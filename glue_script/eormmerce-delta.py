...
def partition_data(df, partition_by, num_partitions=None, sort_by=None):
    ...
    try:
        if num_partitions and sort_by:
            ...
            df = df.repartition(num_partitions, col(partition_by)).sortWithinPartitions(*sort_columns)
        elif num_partitions:
...
def save_normalized_data_to_s3(df, bucket_name, output_path, archive_path, partition_col=None):
    ...
    try:
        ...
        if partition_col and df.schema.names != [partition_col]:
            df.write.format("delta").partitionBy(partition_col).mode("overwrite").save(primary_full_path)
        else:
        ...
        if partition_col and df.schema.names != [partition_col]:
            df.write.format("delta").partitionBy(partition_col).mode("overwrite").save(archive_full_path)
        else:
...
def save_master_data_to_s3(df, bucket_name, output_path, archive_path):
    ...
    try:
        primary_full_path = f"s3a://{bucket_name}/{output_path}"
        df.write.format("delta").partitionBy("date").mode("overwrite").save(primary_full_path)
        logger.info(f"Successfully saved master data to {primary_full_path}")
        
        archive_full_path = f"s3a://{bucket_name}/{archive_path}"
        df.write.format("delta").partitionBy("date").mode("overwrite").save(archive_full_path)
...
def main():
    ...
    try:
        ...
        # Load data
        order_items_df = load_data_from_s3(spark, bucket_name, "raw-data/order_items_apr_2025/")
        ...
        # Save master data
        save_master_data_to_s3(final_df, bucket_name, "lakehouse-dwh/master/", f"{archive_path}/master/")

        # Normalize and save dimensional tables
        users_df, departments_df, products_df, orders_df, order_items_df = normalize_data(final_df)

        # Save normalized data
        save_normalized_data_to_s3(users_df, bucket_name, "lakehouse-dwh/users/", f"{archive_path}/users/")
        save_normalized_data_to_s3(departments_df, bucket_name, "lakehouse-dwh/departments/", f"{archive_path}/departments/", "department_id")
        save_normalized_data_to_s3(products_df, bucket_name, "lakehouse-dwh/products/", f"{archive_path}/products/", "department_id")
        save_normalized_data_to_s3(orders_df, bucket_name, "lakehouse-dwh/orders/", f"{archive_path}/orders/", "date")
        save_normalized_data_to_s3(order_items_df, bucket_name, "lakehouse-dwh/order_items/", f"{archive_path}/order_items/", "date")
